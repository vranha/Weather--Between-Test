...
app.get("/api/weather/:date&:location", async (req, res) => {
    try {
        ...
        const queryRef = citiesRef
            ...
            .where("location.city", "==", req.params.location);
        const docs = await queryRef.get();
...
// Cambiar solo porcentaje de lluvia de una entrada
app.patch("/api/weather/:weather_id", async (req, res) => {
    try {
